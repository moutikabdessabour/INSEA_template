...
\section{Adding chapters or sections}
...
\end{minted} 
\linespread{1.5}\section{Headings and subsections}
Within each section one creates heading by using the command \mintinline[bgcolor=lightgray,style=vs]{latex}{\section{Heading here}}. The subsections you create will automatically be added to the table of contents. One can have different levels of headings using \mintinline[bgcolor=lightgray,style=vs]{latex}{\subsubsection{}} or \mintinline[bgcolor=lightgray,style=vs]{latex}{\paragraph{}}. The different headings will be marked X.1 for subsections, X.1.1 for subsubsections and X1.1.1 for paragraphs, where X represents the chapters.
...
Tables can be designed any way you like. Tables are numbered the same way as figures and can also be referenced directly if you give them a label. There are several \LaTeX{} table generators online, for example \url{https://www.tablesgenerator.com/}, which work very well. There are also some packages in R mainly xtable/kable which transforms R-data.frames to \LaTeX{} tables/code. This can be very helpful and save a lot of time if you are working with R. The following code produces Table \ref{tab:Variables}.
